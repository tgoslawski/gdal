    ...
    switch (nEPSG)
    {
        ...
        default:
        {
            CPLError(CE_Warning, CPLE_AppDefined,
                     ...
                     "file: %i. Defaulting to EPSG:4326/SOSI 84.",
                     nEPSG);
        }
    }
    return nSOSI;
...
OGRSOSIDataSource::~OGRSOSIDataSource()
{
    ...
    if (poPolyHeaders != nullptr)
        delete poPolyHeaders;
    if (poTextHeaders != nullptr)
        delete poTextHeaders;
    if (poPointHeaders != nullptr)
        delete poPointHeaders;
    if (poCurveHeaders != nullptr)
        delete poCurveHeaders;

    if (nMode == MODE_WRITING)
    {
        if (poFileadm != nullptr)
            LC_CloseSos(poFileadm, RESET_IDX);
        if (poBaseadm != nullptr)
            LC_CloseBase(poBaseadm, RESET_IDX);
    }
    else
    {
        if (poFileadm != nullptr)
            LC_CloseSos(poFileadm, SAVE_IDX);
        if (poBaseadm != nullptr)
            LC_CloseBase(poBaseadm, SAVE_IDX);
    }
    poFileadm = nullptr;
    poBaseadm = nullptr;
...
static OGRFeatureDefn *defineLayer(const char *szName,
                                   OGRwkbGeometryType szType, S2I *poHeaders,
                                   S2I **ppoHeadersNew)
{
    ...
    for (S2I::iterator i = poHeaders->begin(); i != poHeaders->end(); ++i)
    {
        ...
        for (int k = 0; k < poType->getElementCount(); k++)
        {
            if (strcmp(poElements[k].GetName(), "") == 0)
                continue;
            OGRFieldDefn oFieldTemplate(poElements[k].GetName(),
                                        poElements[k].GetType());
            (*poHeadersNew)[CPLString(poElements[k].GetName())] =
...
int OGRSOSIDataSource::Open(const char *pszFilename, int bUpdate)
{
    ...
    bool bPointLayer =
        FALSE; /* Initialize four layers for the different geometry types */
    ...
    while (LC_NextBgr(poNextSerial, LC_FRAMGR))
    {
        ...
        nName =
            LC_RxGr(poNextSerial, LES_OPTIMALT, &nNumLines, &nNumCoo, &nInfo);
        ...
        for (short i = 1; i <= nNumLines; i++)
        {
            ...
            if ((pszLine[0] == ':') || (pszLine[0] == '('))
                continue; /* If we have a continued REF line, skip it. */
            if (pszLine[0] == '!')
                continue; /* If we have a comment line, skip it. */

            char *pszUTFLine = CPLRecode(
                pszLine, pszEncoding, CPL_ENC_UTF8); /* switch to UTF encoding
                                                        here, if it is known. */
            char *pszUTFLineIter = pszUTFLine;
            while (pszUTFLineIter[0] == '.')
                pszUTFLineIter++; /* Skipping the dots at the beginning of a
                                     SOSI line */
            char *pszPos2 =
            ...
            if (pszPos2 != nullptr)
            {
                ...
                { /* Add to header list for the corresponding layer, if it is
                    ...
                    case L_LINJE:
                    case L_BUEP:
                    { /* FIXME: maybe not use the same headers for both */
                        if (poCurveHeaders->find(osKey) ==
                            poCurveHeaders->end())
                        {
        ...
        switch (nName)
        {
            ...
            case L_BUEP:
            {
                /* Pre-build a line feature as interpolation from an arc.
                 * Activate line/curve layer. */
                bCurveLayer = TRUE;
            ...
            case L_TEKST:
            {
                 ...
                 */
                /* Todo: observe only points 2ff if more than one point is given
                 * for follow mode */
                bTextLayer = TRUE;
            ...
            case L_HODE:
            {
                ...
                {
                    CPLError(CE_Failure, CPLE_OpenFailed,
                             "TRANSPAR section not found - No reference system "
                ...
                {
                    CPLError(CE_Failure, CPLE_OpenFailed,
                             "OGR could not load coordinate system definition "
                             "EPSG:%i.",
                             nEPSG);
                    return FALSE;
                }
    ...
    papoLayers =
        (OGRSOSILayer **)VSI_MALLOC2_VERBOSE(sizeof(void *), l_nLayers);
    if (papoLayers == nullptr)
    {
        this->nLayers = 0;
        return FALSE;
    ...
    if (bPolyLayer)
    {
        ...
        OGRFeatureDefn *poFeatureDefn =
            defineLayer("polygons", wkbPolygon, poPolyHeaders, &poHeadersNew);
        delete poPolyHeaders;
        poPolyHeaders = poHeadersNew;
        poFeatureDefn->Reference();
        papoLayers[--l_nLayers] =
            new OGRSOSILayer(this, poFeatureDefn, poFileadm, poPolyHeaders);
    }
    else
    {
        delete poPolyHeaders;
    ...
    if (bCurveLayer)
    {
        S2I *poHeadersNew = new S2I();
        OGRFeatureDefn *poFeatureDefn =
            defineLayer("lines", wkbLineString, poCurveHeaders, &poHeadersNew);
        delete poCurveHeaders;
        poCurveHeaders = poHeadersNew;
        poFeatureDefn->Reference();
        papoLayers[--l_nLayers] =
            new OGRSOSILayer(this, poFeatureDefn, poFileadm, poCurveHeaders);
    }
    else
    {
        delete poCurveHeaders;
    ...
    if (bPointLayer)
    {
        S2I *poHeadersNew = new S2I();
        OGRFeatureDefn *poFeatureDefn =
            defineLayer("points", wkbPoint, poPointHeaders, &poHeadersNew);
        delete poPointHeaders;
        poPointHeaders = poHeadersNew;
        poFeatureDefn->Reference();
        papoLayers[--l_nLayers] =
            new OGRSOSILayer(this, poFeatureDefn, poFileadm, poPointHeaders);
    }
    else
    {
        delete poPointHeaders;
    ...
    if (bTextLayer)
    {
        S2I *poHeadersNew = new S2I();
        OGRFeatureDefn *poFeatureDefn =
            defineLayer("text", wkbMultiPoint, poTextHeaders, &poHeadersNew);
        delete poTextHeaders;
        poTextHeaders = poHeadersNew;
        poFeatureDefn->Reference();
        papoLayers[--l_nLayers] =
            new OGRSOSILayer(this, poFeatureDefn, poFileadm, poTextHeaders);
    }
    else
    {
        delete poTextHeaders;
...
int OGRSOSIDataSource::Create(const char *pszFilename)
{
    ...
    {
        CPLError(CE_Failure, CPLE_OpenFailed,
                 "Could not open SOSI file for writing (Status %i).",
                 nDetStatus);
        return FALSE;
    }
    ...
    OGRwkbGeometryType eGType, CPL_UNUSED char **papszOptions)
{
    ...
    if (poSRS == NULL)
    {
        if (poSpatialRef != NULL)
        {
            ...
            if (pszKoosys == NULL)
            {
                ...
                {
                    CPLError(CE_Failure, CPLE_OpenFailed,
                             "Could not identify EPSG code for spatial "
                             "reference system");
                    return NULL;
                }
            ...
            else
            {
                ...
                {
                    CPLError(CE_Failure, CPLE_OpenFailed,
                             "Could not retrieve EPSG code for spatial "
                             "reference system");
                    return NULL;
                }
            }
    ...
    else
    {
        if (!poSRS->IsSame(poSpatialRef))
        {
            CPLError(CE_Failure, CPLE_AppDefined,
                     "SOSI driver does not support different spatial reference "
                     "systems in one file.");
        }
    }
    ...
    OGRSOSILayer *poLayer =
        new OGRSOSILayer(this, poFeatureDefn, poFileadm, NULL /*poHeaderDefn*/);
